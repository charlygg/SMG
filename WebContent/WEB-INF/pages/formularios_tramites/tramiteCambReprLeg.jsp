...
$("#idtramite_cambio_representante")
.change(
		function() {
			$
					.getJSON(
							'consultaTramite_cambio_representante?idtramite_cambio_representante='
									+ $(
											"#idtramite_cambio_representante")
											.val(),
							function(beanDatos) {
...
$(document).ready(function() {
		$.validator.setDefaults({submitHandler: function() { transaccionDatosRetornoID( 'registerTramite_cambio_representante', 'formulario', true, "exito()", "fallo()");}});
		$('#formulario').validate();
		comboListaTramite();
		comboEstatus();
	});
...
$('#idtramite_cambio_representante').empty();
$.getJSON('listacomboTramite_cambio_representante?rol_usuario='
		+ $("#rol_usuario").val(), function(beanDatos) {
	console.log(JSON.stringify(beanDatos));
	//datos para el combo
	if ($("#rol_usuario").val() != 'ROLE_RECEP') {
		$("#idtramite_cambio_representante").append(
	...
	$.each(beanDatos, function(i, item) {
		console.log(item.idtramite_cambio_representante);
		var estatus = '';
		switch (item.estatus) {
		case 1:
			estatus = 'Nuevo';
		...
		$("#idtramite_cambio_representante").append(
				"<option value=\""+item.idtramite_cambio_representante+"\">Trámite No "
						+ item.idtramite_cambio_representante
	...
	function exito(){
// 		mensajeriaAlertas('Exito', 'En Alta de Tramite Cambio de Representante Legal');
		comboListaTramite();
		$('#idtramite_cambio_representante').eq(1).prop('selected', true);
	...
	function fallo(){
		mensajeriaAlertas('Error', 'En Alta de Tramite Cambio de Representante Legal');
		$('#idtramite_cambio_representante').eq(1).prop('selected', true);
		$('#catalogo_perfilempresa_idcatalogo_perfilempresa').eq(0).prop(
				'selected', true);
		$("#catalogo_perfilestablecimiento_idcatalogo_perfilestablecimiento")
	...
	function cargaTablaMovsBitacora(estatus){
		if(estatus == 1){
			...
			$('#divCargaBitacora').load('consultaBitacoraCatReg?idregistro='+$('#idtramite_cambio_representante').val()+'&catalogoID='+$('#tramite_catalogoID').val());
		}
	...
	<div class="col-lg-12">
		<div class="box box-success">
			<div class="box-header with-border">
				<h3 class="box-title">Tramite Cambio de Representante Legal.</h3>
			</div>
			...
			<div class="box-body">
				...
				<form id="formulario" role="form">
						...
						<c:when test="${empty data}">
							<div class="input-group" id="dividtramite">
								<span class="input-group-addon">Trámites en Ejecución</span> <select
									class="form-control" id="idtramite_cambio_representante"
									name="idtramite_cambio_representante"></select>
							</div>
							<br>
					...
					<!--  -->
					<div class="input-group">
						<span class="input-group-addon">Nuevo representante Legal</span>
						<c:choose>
							<c:when test="${!empty data}">
								<input maxlength="100" type="text" class="form-control"
									value="${data.nuevo_representante}" id="nuevo_representante"
									name="nuevo_representante" required minlength="2">
							</c:when>
							<c:otherwise>
								<input maxlength="100" type="text" class="form-control"
									id="nuevo_representante" name="nuevo_representante" required
									minlength="2">
							</c:otherwise>
						</c:choose>
