...
$("#idtramite_cierre_operaciones")
.change(
		function() {
			$
					.getJSON(
							'consultaTramite_cierre_operaciones?idtramite_cierre_operaciones='
									+ $(
											"#idtramite_cierre_operaciones")
											.val(),
							function(beanDatos) {
...
$(document).ready(function() {
		$.validator.setDefaults({submitHandler: function() { transaccionDatosRetornoID( 'registerTramite_cierre_operaciones', 'formulario', true, "exito()", "fallo()");}});
		$('#formulario').validate();
		comboListaTramite();
		comboEstatus();
	});
...
$('#idtramite_cierre_operaciones').empty();
$.getJSON('listacomboTramite_cierre_operaciones?rol_usuario='
		+ $("#rol_usuario").val(), function(beanDatos) {
	console.log(JSON.stringify(beanDatos));
	//datos para el combo
	if ($("#rol_usuario").val() != 'ROLE_RECEP') {
		$("#idtramite_cierre_operaciones").append(
	...
	$.each(beanDatos, function(i, item) {
		console.log(item.idtramite_cierre_operaciones);
		var estatus = '';
		switch (item.estatus) {
		case 1:
			estatus = 'Nuevo';
		...
		$("#idtramite_cierre_operaciones").append(
				"<option value=\""+item.idtramite_cierre_operaciones+"\">Trámite No "
						+ item.idtramite_cierre_operaciones
	...
	function exito(){
// 		mensajeriaAlertas('Exito', 'En Alta de Tramite Cierre de operaciones');
		comboListaTramite();
		$('#idtramite_cierre_operaciones').eq(1).prop('selected', true);
	...
	function fallo(){
		mensajeriaAlertas('Error', 'En Alta de Tramite Cierre de operaciones');
		$('#idtramite_cierre_operaciones').eq(1).prop('selected', true);
		$('#catalogo_perfilempresa_idcatalogo_perfilempresa').eq(0).prop(
				'selected', true);
		$("#catalogo_perfilestablecimiento_idcatalogo_perfilestablecimiento")
	...
	function cargaTablaMovsBitacora(estatus){
		if(estatus == 1){
			...
			$('#divCargaBitacora').load('consultaBitacoraCatReg?idregistro='+$('#idtramite_cierre_operaciones').val()+'&catalogoID='+$('#tramite_catalogoID').val());
		}
	...
	<div class="col-lg-12">
		<div class="box box-success">
			<div class="box-header with-border">
				<h3 class="box-title">Tramite Cierre de operaciones.</h3>
			</div>
			<div class="box-body">
				<!-- elemntos que se cambian por tramite -->
				<input type="hidden" id="areaNumOficio" value="DJ"> <input
					type="hidden" id="titleNumOficio"
					value="Tramite Cierre de operaciones"> <input type="hidden"
					id="tipoTramiteNumOficio" value="5">
				<%@ include
						...
						<c:when test="${empty data}">
							<div class="input-group" id="dividtramite">
								<span class="input-group-addon">Trámites en Ejecución</span> <select
									class="form-control" id="idtramite_cierre_operaciones"
									name="idtramite_cierre_operaciones"></select>
							</div>
							<br>
						</c:when>
					</c:choose>
