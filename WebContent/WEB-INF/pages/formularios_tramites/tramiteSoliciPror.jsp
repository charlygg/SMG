...
$("#idtramite_solicitud_prorroga")
.change(
		function() {
			$.getJSON('consultaTramite_solicitud_prorroga?idtramite_solicitud_prorroga='+ $("#idtramite_solicitud_prorroga").val(),function(beanDatos) {
...
$(document).ready(function() {
		$.validator.setDefaults({submitHandler: function() { transaccionDatosRetornoID( 'registerTramite_solicitud_prorroga', 'formulario', true, "exito()", "fallo()");}});
		$('#formulario').validate();
		comboListaTramite();
		comboEstatus();
	});
...
$('#idtramite_solicitud_prorroga').empty();
$.getJSON('listacomboTramite_solicitud_prorroga?rol_usuario='
		+ $("#rol_usuario").val(), function(beanDatos) {
	console.log(JSON.stringify(beanDatos));
	//datos para el combo
	if ($("#rol_usuario").val() != 'ROLE_RECEP') {
		$("#idtramite_solicitud_prorroga").append(
	...
	$.each(beanDatos, function(i, item) {
		console.log(item.idtramite_solicitud_prorroga);
		var estatus = '';
		switch (item.estatus) {
		case 1:
			estatus = 'Nuevo';
		...
		$("#idtramite_solicitud_prorroga").append(
				"<option value=\""+item.idtramite_solicitud_prorroga+"\">Trámite No "
						+ item.idtramite_solicitud_prorroga
	...
	function exito(){
// 		mensajeriaAlertas('Exito', 'En Alta de Solicitud de prorrogas');
		comboListaTramite();
		$('#idtramite_solicitud_prorroga').eq(1).prop('selected', true);
	...
	function fallo(){
		mensajeriaAlertas('Error', 'En Alta de Solicitud de prorrogas');
		$('#idtramite_solicitud_prorroga').eq(1).prop('selected', true);
		$('#catalogo_perfilempresa_idcatalogo_perfilempresa').eq(0).prop(
				'selected', true);
		$("#catalogo_perfilestablecimiento_idcatalogo_perfilestablecimiento")
	...
	function cargaTablaMovsBitacora(estatus){
		if(estatus == 1){
			...
			$('#divCargaBitacora').load('consultaBitacoraCatReg?idregistro='+$('#idtramite_solicitud_prorroga').val()+'&catalogoID='+$('#tramite_catalogoID').val());
		}
	...
	<div class="col-lg-12">
		<div class="box box-success">
			<div class="box-header with-border">
				<h3 class="box-title">Tramite Solicitud de prorrogas.</h3>
			</div>
			<div class="box-body">
				<!-- elemntos que se cambian por tramite -->
				<input type="hidden" id="areaNumOficio" value="IA"> <input
					type="hidden" id="titleNumOficio"
					value="Tramite Solicitud de prorrogas"> <input
					type="hidden" id="tipoTramiteNumOficio" value="9">
				<%@ include
						...
						<c:when test="${empty data}">
							<div class="input-group" id="dividtramite">
								<span class="input-group-addon">Trámites en Ejecución</span> <select
									class="form-control" id="idtramite_solicitud_prorroga"
									name="idtramite_solicitud_prorroga"></select>
							</div>
							<br>
						</c:when>
					</c:choose>
