...
$("#idtramite_modificacion_proyecto")
.change(
		function() {
			$
					.getJSON(
							'consultaTramite_modificacion_proyecto?idtramite_modificacion_proyecto='
									+ $(
											"#idtramite_modificacion_proyecto")
											.val(),
							function(beanDatos) {
...
$(document).ready(function() {
		$.validator.setDefaults({submitHandler: function() { transaccionDatosRetornoID( 'registerTramite_modificacion_proyecto', 'formulario', true, "exito()", "fallo()");}});
		$('#formulario').validate();
		comboListaTramite();
		comboEstatus();
	});
...
$('#idtramite_modificacion_proyecto').empty();
$.getJSON('listacomboTramite_modificacion_proyecto?rol_usuario='
		+ $("#rol_usuario").val(), function(beanDatos) {
	console.log(JSON.stringify(beanDatos));
	//datos para el combo
	if ($("#rol_usuario").val() != 'ROLE_RECEP') {
		$("#idtramite_modificacion_proyecto").append(
	...
	$.each(beanDatos, function(i, item) {
		console.log(item.idtramite_modificacion_proyecto);
		var estatus = '';
		switch (item.estatus) {
		case 1:
			estatus = 'Nuevo';
		...
		$("#idtramite_modificacion_proyecto").append(
				"<option value=\""+item.idtramite_modificacion_proyecto+"\">Trámite No "
						+ item.idtramite_modificacion_proyecto
	...
	function exito(){
// 		mensajeriaAlertas('Exito', 'En Alta de Tramite Modificación de proyectos');
		comboListaTramite();
		$('#idtramite_modificacion_proyecto').eq(1).prop('selected', true);
	...
	function fallo(){
		mensajeriaAlertas('Error', 'En Alta de Tramite Modificación de proyectos');
		$('#idtramite_modificacion_proyecto').eq(1).prop('selected', true);
		$('#catalogo_perfilempresa_idcatalogo_perfilempresa').eq(0).prop(
				'selected', true);
		$("#catalogo_perfilestablecimiento_idcatalogo_perfilestablecimiento")
	...
	function cargaTablaMovsBitacora(estatus){
		if(estatus == 1){
			...
			$('#divCargaBitacora').load('consultaBitacoraCatReg?idregistro='+$('#idtramite_modificacion_proyecto').val()+'&catalogoID='+$('#tramite_catalogoID').val());
		}
	...
	<div class="col-lg-12">
		<div class="box box-success">
			<div class="box-header with-border">
				<h3 class="box-title">Tramite Modificación de proyectos.</h3>
			</div>
			...
			<div class="box-body">
				...
				<input type="hidden" id="areaNumOficio" value="IA"> <input
					...
					value="Tramite Modificación de proyectos"> <input
					type="hidden" id="tipoTramiteNumOficio" value="12">
				<%@ include
						...
						<c:when test="${empty data}">
							<div class="input-group" id="dividtramite">
								<span class="input-group-addon">Trámites en Ejecución</span> <select
									class="form-control" id="idtramite_modificacion_proyecto"
									name="idtramite_modificacion_proyecto"></select>
							</div>
							<br>
						</c:when>
					</c:choose>
