...
$("#catalogo_perfilempresa_idcatalogo_perfilempresa").change(function() {
 ...
 $("#idtramite_reception_validacion_informacion").change(function() {
	    $.getJSON('consultaTramite_reception_validacion_informacion?idtramite_reception_validacion_informacion='+$("#idtramite_reception_validacion_informacion").val(),function(data){
	        console.log(JSON.stringify(data));
		...
		function comboListaTramite(){
			$.getJSON('listacomboTramite_reception_validacion_informacion?rol_usuario='+$("#rol_usuario").val(),function(data){
		        console.log(JSON.stringify(data));
		        //datos para el combo
		        if($("#rol_usuario").val() != 'ROLE_RECEP'){
		        	$("#idtramite_reception_validacion_informacion").append("<option value='0'>Escoja una opción</option>");	
		        }else{
		        	$("#idtramite_reception_validacion_informacion").append("<option value='0'>Escoja una opción</option>");
		        }
		        
		        $.each(data, function(i,item){
		        	 console.log(item.idtramite_reception_validacion_informacion);
		        	 var estatus = ''; 
		        	switch(item.estatus){
		            ...
		            $("#idtramite_reception_validacion_informacion").append("<option value=\""+item.idtramite_reception_validacion_informacion+"\">Trámite No "+item.idtramite_reception_validacion_informacion+" en Estatus "+estatus+"</option>");
		        });
...
$(document).ready(function() {
		$.validator.setDefaults({submitHandler: function() { transaccionDatosRetornoID( 'registerTramite_reception_validacion_informacion', 'formulario', true, "exito()", "fallo()");}});
		$('#formulario').validate();
		comboListaTramite();
		comboEstatus();
});
	...
	function exito(){
// 		mensajeriaAlertas('Exito', 'En Alta de Tramite Recepción y Validación de información del COA');
	...
	function fallo(){
		mensajeriaAlertas('Error', 'En Alta de Tramite Recepción y Validación de información del COA');
		document.getElementById("catalogo_perfilempresa_idcatalogo_perfilempresa").selectedIndex = 0;
		document.getElementById("catalogo_perfilestablecimiento_idcatalogo_perfilestablecimiento").selectedIndex = -1;
		document.getElementById("estatus").selectedIndex = 0;
		$('#divCargaBitacora').hide();
	...
	if($("#busquedaTramite").val() != 0){
	    ...
	    $.getJSON('consultaTramite_reception_validacion_informacion?idtramite_reception_validacion_informacion='+$("#busquedaTramite").val(),function(data){
	        console.log(JSON.stringify(data));
	        //cargar datos en la vista:
	        	$('#catalogo_perfilempresa_idcatalogo_perfilempresa > option[value="'+data.catalogo_perfilempresa_idcatalogo_perfilempresa+'"]').attr('selected', 'selected');
	        	///LLenas catalogo de establecimientos
	...
	function cargaTablaMovsBitacora(estatus){
		if(estatus == 1){
			...
			$('#divCargaBitacora').load('consultaBitacoraCatReg?idregistro='+$('#idtramite_reception_validacion_informacion').val()+'&catalogoID='+$('#tramite_catalogoID').val());
		}
	...
	<div class="col-lg-12">
		<div class="box box-success">
			<div class="box-header with-border">
				<h3 class="box-title">Tramite Recepción y Validación de
					información del COA</h3>
			</div>
			<form id="formulario" role="form">
				<div class="box-body">
						...
						<c:when test="${empty data}">
							<div class="input-group" id="dividtramite">
								<span class="input-group-addon">Trámites en Ejecución</span> <select
									class="form-control"
									id="idtramite_reception_validacion_informacion"
									name="idtramite_reception_validacion_informacion"></select>
							</div>
						</c:when>
					</c:choose>
					...
					<br> <input type="hidden"
						id="idtramite_reception_validacion_informacion"
						name="idtramite_reception_validacion_informacion" value="0">
					<!--  -->
					<c:choose>
						<c:when test="${empty data}">
						...
						<c:otherwise>
							...
							<input type="hidden" id="busquedaTramite" name="busquedaTramite"
								value="${data.idtramite_reception_validacion_informacion}">
						</c:otherwise>
					</c:choose>
					<!--  -->
					<!-- Termina Agregar -->
