...
$("#f_tramite_concluido").datepicker({dateFormat: "dd/mm/yy"});
										...
										function(beanDatos) {
											...
											$
													.each(
															...
															function(i, item) {
																console
																		.log(item.idcatalogo_perfilestablecimiento);
																$(
																		"#catalogo_perfilestablecimiento_idcatalogo_perfilestablecimiento")
																		.append(
	...
	function llenarcatalogo_perfilestablecimiento_idcatalogo_perfilestablecimiento(datos){
		$.getJSON('getEstablecimientosByID?idestablecimiento='+datos , function(
		beanDatos) {
	console.log(JSON
			.stringify(beanDatos));
	$('#calleEstablecimiento').val(beanDatos.calle);
    $('#numeroExteriorEstablecimiento').val(beanDatos.numeroExterior);
    $('#numeroInteriorEstablecimiento').val(beanDatos.numeroInterior);
    $('#coloniaEstablecimiento').val(beanDatos.colonia);
    $('#municipioEstablecimiento').val(beanDatos.municipio);
    $('#estadoEstablecimiento').val(beanDatos.estado);
    $('#latitudEstablecimiento').val(beanDatos.latitud);
    $('#longitudEstablecimiento').val(beanDatos.longitud);
	$('#DatosEstablecimiento').prop(
			"hidden", false);
});
	}
	$("#idtramite_cambio_razon_social")
			.change(
					function() {
						$
								.getJSON(
										'consultaTramite_cambio_razon_social?idtramite_cambio_razon_social='
												+ $(
														"#idtramite_cambio_razon_social")
														.val(),
										function(beanDatos) {
			...
			function() {
				$.validator.setDefaults({
					submitHandler : function() {
						transaccionDatosRetornoID('registerTramite_cambio_razon_social',
	...
	function comboListaTramite() {
		$('#idtramite_cambio_razon_social').empty();
		$.getJSON('listacomboTramite_cambio_razon_social?rol_usuario='
				+ $("#rol_usuario").val(), function(beanDatos) {
			console.log(JSON.stringify(beanDatos));
			//datos para el combo
			if ($("#rol_usuario").val() != 'ROLE_RECEP') {
				$("#idtramite_cambio_razon_social").append(
						"<option value=''>Escoja una opción</option>");
			} else {
				$("#idtramite_cambio_razon_social").append(
			...
			$.each(beanDatos, function(i, item) {
				console.log(item.idtramite_cambio_razon_social);
				var estatus = '';
				switch (item.estatus) {
				case 1:
					estatus = 'Nuevo';
				...
				$("#idtramite_cambio_razon_social").append(
						"<option value=\""+item.idtramite_cambio_razon_social+"\">Trámite No "
								+ item.idtramite_cambio_razon_social
	...
	function exito() {
// 		mensajeriaAlertas('Exito', 'En Alta de Tramite Cambio de Razón Social');
		comboListaTramite();
		$('#idtramite_cambio_razon_social').eq(1).prop('selected', true);
	...
	function fallo() {
		mensajeriaAlertas('Error', 'En Alta de Tramite Cambio de Razón Social');
		$('#idtramite_cambio_razon_social').eq(1).prop('selected', true);
		$('#catalogo_perfilempresa_idcatalogo_perfilempresa').eq(0).prop(
				'selected', true);
		$("#catalogo_perfilestablecimiento_idcatalogo_perfilestablecimiento")
	...
	function cargaTablaMovsBitacora(estatus){
		if(estatus == 1){
			...
			$('#divCargaBitacora').load('consultaBitacoraCatReg?idregistro='+$('#idtramite_cambio_razon_social').val()+'&catalogoID='+$('#tramite_catalogoID').val());
		}
	...
	<div class="col-lg-12">
		<div class="box box-success">
			<div class="box-header with-border">
				<h3 class="box-title">Tramite Cambio de Razón Social.</h3>
			</div>
			...
			<div class="box-body">
				...
				<input type="hidden" id="areaNumOficio" value="DJ"> <input
					...
					value="Tramite Cambio de Razón Social"> <input
					type="hidden" id="tipoTramiteNumOficio" value="3">
				<%@ include
				...
				<form id="formulario" role="form">
						...
						<c:when test="${empty data}">
							<div class="input-group" id="dividtramite">
								<span class="input-group-addon">Trámites en Ejecución</span> <select
									class="form-control" id="idtramite_cambio_razon_social"
									name="idtramite_cambio_razon_social"></select>
							</div>
							<br>
						</c:when>
					</c:choose>
					...
					<!--  -->
					<div class="input-group">
						<span class="input-group-addon">Nueva razón social</span>
						<c:choose>
							<c:when test="${!empty data}">
								<textarea class="form-control" id="nueva_razon"
									name="nueva_razon" required>${data.nueva_razon}"</textarea>
							</c:when>
							<c:otherwise>
								<textarea class="form-control" id="nueva_razon"
									name="nueva_razon" required></textarea>
							</c:otherwise>
						</c:choose>
					</div>
					<br>
