 ...
 */
 ...
 $("#idtramite_rehuso_residuos_manejo_especial").change(function() {
	    $.getJSON('consultaTramite_rehuso_residuos_manejo_especial?idtramite_rehuso_residuos_manejo_especial='+$("#idtramite_rehuso_residuos_manejo_especial").val(),function(data){
	        console.log(JSON.stringify(data));
		...
		function comboListaTramite(){
			$.getJSON('listacomboTramite_rehuso_residuos_manejo_especial?rol_usuario='+$("#rol_usuario").val(),function(data){
		        console.log(JSON.stringify(data));
		        //datos para el combo
		        if($("#rol_usuario").val() != 'ROLE_RECEP'){
		        	$("#idtramite_rehuso_residuos_manejo_especial").append("<option value='0'>Escoja una opción</option>");	
		        }else{
		        	$("#idtramite_rehuso_residuos_manejo_especial").append("<option value='0'>Escoja una opción</option>");
		        }
		        
		        $.each(data, function(i,item){
		        	 console.log(item.idtramite_rehuso_residuos_manejo_especial);
		        	 var estatus = ''; 
		        	switch(item.estatus){
		            ...
		            $("#idtramite_rehuso_residuos_manejo_especial").append("<option value=\""+item.idtramite_rehuso_residuos_manejo_especial+"\">Trámite No "+item.idtramite_rehuso_residuos_manejo_especial+" en Estatus "+estatus+"</option>");
		        });
...
$(document).ready(function() {
		$.validator.setDefaults({submitHandler: function() { transaccionDatosRetornoID( 'registerTramite_rehuso_residuos_manejo_especial', 'formulario', true, "exito()", "fallo()");}});
		$('#formulario').validate();
		comboListaTramite();
		comboEstatus();
});
	...
	function exito(){
// 		mensajeriaAlertas('Exito', 'En Alta de Tramite Reuso de residuos de manejo especial.');
	...
	function fallo(){
		mensajeriaAlertas('Error', 'En Alta de Tramite Reuso de residuos de manejo especial.');
		document.getElementById("catalogo_perfilempresa_idcatalogo_perfilempresa").selectedIndex = 0;
		document.getElementById("catalogo_perfilestablecimiento_idcatalogo_perfilestablecimiento").selectedIndex = -1;
		document.getElementById("catalogo_perfilestablecimiento_idcatalogo_perfilestablecimiento1").selectedIndex = -1;
		document.getElementById("estatus").selectedIndex = 0;
	...
	if($("#busquedaTramite").val() != 0){
		  $.getJSON('consultaTramite_rehuso_residuos_manejo_especial?idtramite_rehuso_residuos_manejo_especial='+$("#busquedaTramite").val(),function(data){
		        console.log(JSON.stringify(data));
		        //cargar datos en la vista:
		        	$('#catalogo_perfilempresa_idcatalogo_perfilempresa > option[value="'+data.catalogo_perfilempresa_idcatalogo_perfilempresa+'"]').attr('selected', 'selected');
		        	///LLenas catalogo de establecimientos
	...
	function cargaTablaMovsBitacora(estatus){
		if(estatus == 1){
			...
			$('#divCargaBitacora').load('consultaBitacoraCatReg?idregistro='+$('#idtramite_rehuso_residuos_manejo_especial').val()+'&catalogoID='+$('#tramite_catalogoID').val());
		}
	...
	<div class="col-lg-12">
		<div class="box box-success">
			<div class="box-header with-border">
				<h3 class="box-title">Reuso de residuos de manejo especial.</h3>
			</div>
			<div class="box-body">
				<!-- elemntos que se cambian por tramite -->
				<input type="hidden" id="areaNumOficio" value="RME"> <input
					type="hidden" id="titleNumOficio"
					value="Tramite Reuso de residuos de manejo especial"> <input
					type="hidden" id="tipoTramiteNumOficio" value="40">
				<%@ include
				...
				<form id="formulario" role="form">
					...
					<div class="input-group" id="dividtramite">
						<span class="input-group-addon">Trámites en Ejecución</span> <select
							class="form-control"
							id="idtramite_rehuso_residuos_manejo_especial"
							name="idtramite_rehuso_residuos_manejo_especial"></select>
					</div>
					<br>
					...
					<br> <input type="hidden"
						id="idtramite_rehuso_residuos_manejo_especial"
						name="idtramite_rehuso_residuos_manejo_especial" value="0">
					<!--  -->
					<c:choose>
						<c:when test="${empty data}">
						...
						<c:otherwise>
							...
							<input type="hidden" id="busquedaTramite" name="busquedaTramite"
								value="${data.idtramite_rehuso_residuos_manejo_especial}">
						</c:otherwise>
					</c:choose>
					<!--  -->
					<!-- Termina Agregar -->
