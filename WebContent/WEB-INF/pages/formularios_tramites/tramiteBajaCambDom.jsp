...
$("#catalogo_perfilempresa_idcatalogo_perfilempresa").change(function() {
 ...
 $("#idtramite_baja_cambio_dom").change(function() {
	    $.getJSON('consultaTramite_baja_cambio_dom?idtramite_baja_cambio_dom='+$("#idtramite_baja_cambio_dom").val(),function(data){
	        console.log(JSON.stringify(data));
		...
		function comboListaTramite(){
			$.getJSON('listacomboTramite_baja_cambio_dom?rol_usuario='+$("#rol_usuario").val(),function(data){
		        console.log(JSON.stringify(data));
		        //datos para el combo
		        if($("#rol_usuario").val() != 'ROLE_RECEP'){
		        	$("#idtramite_baja_cambio_dom").append("<option value='0'>Escoja una opción</option>");	
		        }else{
		        	$("#idtramite_baja_cambio_dom").append("<option value='0'>Escoja una opción</option>");
		        }
		        
		        $.each(data, function(i,item){
		        	 console.log(item.idtramite_baja_cambio_dom);
		        	 var estatus = ''; 
		        	switch(item.estatus){
		            ...
		            $("#idtramite_baja_cambio_dom").append("<option value=\""+item.idtramite_baja_cambio_dom+"\">Trámite No "+item.idtramite_baja_cambio_dom+" en Estatus "+estatus+"</option>");
		        });
...
$(document).ready(function() {
		$.validator.setDefaults({submitHandler: function() { transaccionDatosRetornoID( 'registerTramite_baja_cambio_dom', 'formulario', true, "exito()", "fallo()");}});
		$('#formulario').validate();
		comboListaTramite();
		comboEstatus();
});
	...
	function exito(){
// 		mensajeriaAlertas('Exito', 'En Alta de Tramite Baja por cambio de domicilio');
	...
	function fallo(){
		mensajeriaAlertas('Error', 'En Alta de Tramite Baja por cambio de domicilio');
		document.getElementById("catalogo_perfilempresa_idcatalogo_perfilempresa").selectedIndex = 0;
		document.getElementById("catalogo_perfilestablecimiento_idcatalogo_perfilestablecimiento").selectedIndex = -1;
		document.getElementById("estatus").selectedIndex = 0;
		$('#divCargaBitacora').hide();
	...
	if($("#busquedaTramite").val() != 0){
	    ...
	    $.getJSON('consultaTramite_baja_cambio_dom?idtramite_baja_cambio_dom='+$("#busquedaTramite").val(),function(data){
	        console.log(JSON.stringify(data));
	        //cargar datos en la vista:
	        	$('#catalogo_perfilempresa_idcatalogo_perfilempresa > option[value="'+data.catalogo_perfilempresa_idcatalogo_perfilempresa+'"]').attr('selected', 'selected');
	        	///LLenas catalogo de establecimientos
	...
	function cargaTablaMovsBitacora(estatus){
		if(estatus == 1){
			...
			$('#divCargaBitacora').load('consultaBitacoraCatReg?idregistro='+$('#idtramite_baja_cambio_dom').val()+'&catalogoID='+$('#tramite_catalogoID').val());
		}
	...
	<div class="col-lg-12">
		<div class="box box-success">
			<div class="box-header with-border">
				<h3 class="box-title">Tramite Baja por cambio de domicilio</h3>
			</div>
			...
			<div class="box-body">
				...
				<input type="hidden" id="areaNumOficio" value="CD"> <input
					...
					value="Tramite Baja por cambio de domicilio"> <input
					type="hidden" id="tipoTramiteNumOficio" value="24">
				<%@ include
				...
				<form id="formulario" role="form">
						...
						<c:when test="${empty data}">
							<div class="input-group" id="dividtramite">
								<span class="input-group-addon">Trámites en Ejecución</span> <select
									class="form-control" id="idtramite_baja_cambio_dom"
									name="idtramite_baja_cambio_dom"></select>
							</div>
						</c:when>
					</c:choose>
					<br>
					...
					<br> <input type="hidden" id="idtramite_baja_cambio_dom"
						name="idtramite_baja_cambio_dom" value="0">
					<!--  -->
					<c:choose>
						<c:when test="${empty data}">
						...
						<c:otherwise>
							...
							<input type="hidden" id="busquedaTramite" name="busquedaTramite"
								value="${data.idtramite_baja_cambio_dom}">
						</c:otherwise>
					</c:choose>
					<!--  -->
					<!-- Termina Agregar -->
