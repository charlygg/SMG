...
$("#idtramite").change(function() {
    $.getJSON('consultaTramite_transresidmaespe?idtramite='+$("#idtramite").val(),function(data){
...
$(document).ready(function() {
		$.validator.setDefaults({submitHandler: function() { transaccionDatosRetornoID( 'registerTramite_transresidmaespe', 'formulario', true, "exito()", "fallo()");}});
		$('#formulario').validate();
		comboListaTramite();
		comboEstatus();
});
	...
	function comboListaTramite(){
		$('#idtramite').empty();
		$.getJSON('listacomboTramite_transresidmaespe?rol_usuario='+$("#rol_usuario").val(),function(data){
	        console.log(JSON.stringify(data));
	        //datos para el combo
	        if($("#rol_usuario").val() != 'ROLE_RECEP'){
	...
	function exito(){
// 		mensajeriaAlertas('Exito', 'En Alta de Trámite Transportes de residuos de manejo especial.');
		comboListaTramite();
	...
	function fallo(){
		mensajeriaAlertas('Error', 'En Alta de Trámite Transportes de residuos de manejo especial.');
		$('#DatosEstablecimiento').hide();
		$('#idtramite').eq(1).prop('selected', true);
		$('#idempresa').eq(1).prop('selected', true);
		$("#idestablecimiento").eq(1).prop('selected', true);
	...
	if($("#busquedaTramite").val() != 0){
	    $.getJSON('consultaTramite_transresidmaespe?idtramite='+$("#busquedaTramite").val(),function(data){
	    	$('#idempresa > option[value="'+data.idempresa+'"]').attr('selected', 'selected');
	    	///LLenas catalogo de establecimientos
	    		 $.getJSON('getArrayEstablecimientosPorEmpresa?idempresa='+$("#idempresa").val(),function(lista){
					console.log(JSON.stringify(lista));
		...
		<div class="box box-success">
			...
			<div class="box-body">
				...
				<input type="hidden" id="areaNumOficio" value="RME"> <input
					...
					value="Tramite Transportes de residuos de manejo especial">
				<input type="hidden" id="tipoTramiteNumOficio" value="32">
				<%@ include
				...
				<form id="formulario" role="form">
					...
					<h4>Transportes de residuos de manejo especial.:</h4>

					<h3>Descripción de las características de los vehículos
						utilizados en la recolección .</h3>

					<table data-toggle="table" data-cache="false" data-height="299"
						style="float: center">
						<thead>
							<tr>
								<th data-field="nump">Número de Placa</th>
								<th data-field="niv">NIV</th>
								<th data-field="cap">Capacidad (Toneladas)</th>
								<th data-field="mar">Marca</th>
								<th data-field="tve">Tipo de Vehiculo</th>
								<th data-field="mod">Modelo</th>
							</tr>
						</thead>
						<tbody>
							<tr>
								<td style="text-align: center"><input maxlength="100"
					...
					<div class="input-group">
						<span class="input-group-addon">Número total de vehículos
							recolectores:</span> <input type="text" id="nutoverec" name="nutoverec"
							class="form-control">
					</div>
					<br>
					<h3>Servicios que ofrece la empresa o negociación. (Además de
						la recolección y el transporte).</h3>
