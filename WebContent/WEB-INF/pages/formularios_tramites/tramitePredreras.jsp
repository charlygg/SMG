	...
	<div class="col-lg-12">
		<div class="box box-success">
			<div class="box-header with-border">
				<h3 class="box-title">Trámite Predreras</h3>
			</div>
			<div class="box-body">
					...
					<c:when test="${empty data}">
						<div class="input-group" id="dividtramite">
								<span class="input-group-addon">Trámites en Ejecución</span> <select
									class="form-control" id="idtramite_pedreras"
									name="idtramite_pedreras"></select>
						</div>			
						<br>
...
$(document).ready(function() {
	...
	$("#idtramite_pedreras").change(function() {
			$.getJSON(
				'consultaTramite_pedreras?idtramite_pedreras='
				+ $("#idtramite_pedreras").val(),
							function(beanDatos) {
	...
	$.validator.setDefaults({submitHandler: function() { transaccionDatosRetornoID( 'registerTramite_pedreras', 'formulario', true, "exito()", "fallo()");}});
	$('#formulario').validate();
	comboListaTramite();
	comboEstatus();
});
...
function comboListaTramite(){
	$("#idtramite_pedreras").empty();
	
	$.getJSON('listacomboTramite_pedreras?rol_usuario='
			+ $("#rol_usuario").val(), function(beanDatos) {
		console.log(JSON.stringify(beanDatos));
		//datos para el combo
			if ($("#rol_usuario").val() != 'ROLE_RECEP') {
		$("#idtramite_pedreras").append(
				"<option value=''>Escoja una opción</option>");
	} else {
		$("#idtramite_pedreras").append(
			...
			$.each(beanDatos, function(i, item) {
				console.log(item.idtramite_pedreras);
				var estatus = '';
				switch (item.estatus) {
				case 1:
					estatus = 'Nuevo';
				...
				$("#idtramite_pedreras").append(
						"<option value=\""+item.idtramite_pedreras+"\">Trámite No "
								+ item.idtramite_pedreras
...
function exito(){
	//mensajeriaAlertas('Exito', 'En Alta de Tramite Pedreras');
	comboListaTramite();
	$('#idtramite_pedreras').eq(1).prop('selected', true);
	$('#idempresa').eq(0).prop(
			'selected', true);
	$("#idestablecimiento")
...
function fallo(){
	mensajeriaAlertas('Error', 'En Alta de Tramite Pedreras');
	$('#idtramite_pedreras').eq(1).prop('selected', true);
	$('#idempresa').eq(0).prop(
			'selected', true);
	$("#idestablecimiento")
...
function cargaTablaMovsBitacora(estatus){
	if(estatus == 1){
		...
		$('#divCargaBitacora').load('consultaBitacoraCatReg?idregistro='+$('#idtramite_pedreras').val()+'&catalogoID='+$('#tramite_catalogoID').val());
	}
}
</script>