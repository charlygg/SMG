...
$("#idtramite").change(function() {
    $.getJSON('consultatramite_UbiOperMaIntEscSitDisFiResidProCons?idtramite='+$("#idtramite").val(),function(data){
        ...
        $('#usprteviutesc').val(data.usprteviutesc);
        $('#carecomademades').val(data.carecomademades);
		...
		$(document).ready(function() {
			$.validator.setDefaults({submitHandler: function() { transaccionDatosRetornoID( 'registertramite_UbiOperMaIntEscSitDisFiResidProCons', 'formulario', true, "exito()", "fallo()");}});
			$('#formulario').validate();
			comboListaTramite();
			comboEstatus();
		});
	...
	function comboListaTramite(){
		$('#idtramite').empty();
		$.getJSON('listacombotramite_UbiOperMaIntEscSitDisFiResidProCons?rol_usuario='+$("#rol_usuario").val(),function(data){
	        console.log(JSON.stringify(data));
	        //datos para el combo
	        if($("#rol_usuario").val() != 'ROLE_RECEP'){
	...
	function exito(){
// 		mensajeriaAlertas('Exito', 'En Alta de Tramite Ubicación, operación y manejo integral de escombreras o sitios de disposición final de residuos provenientes de la construcción.');
		comboListaTramite();
	...
	function fallo(){
		mensajeriaAlertas('Error', 'En Alta de Tramite Ubicación, operación y manejo integral de escombreras o sitios de disposición final de residuos provenientes de la construcción.');
		$('#idtramite').eq(1).prop('selected', true);
		$('#idempresa').eq(1).prop('selected', true);
		$("#idestablecimiento").eq(1).prop('selected', true);
		document.getElementById("idempresa").selectedIndex = 0;
	...
	if($("#busquedaTramite").val() != 0){
		 $.getJSON('consultatramite_UbiOperMaIntEscSitDisFiResidProCons?idtramite='+$("#busquedaTramite").val(),function(data){
		    	$('#idempresa > option[value="'+data.idempresa+'"]').attr('selected', 'selected');
		    	///LLenas catalogo de establecimientos
		    		 $.getJSON('getArrayEstablecimientosPorEmpresa?idempresa='+$("#idempresa").val(),function(lista){
						console.log(JSON.stringify(lista));
	...
	<div class="col-lg-12">
		<div class="box box-success">
			<div class="box-header with-border">
				<h3>Ubicación, operación y manejo integral de escombreras o
					sitios de disposición final de residuos provenientes de la
					construcción</h3>
			</div>
			...
			<div class="box-body">
				...
				<input type="hidden" id="areaNumOficio" value="RME"> <input
					...
					value="Tramite Ubicación, operación y manejo integral de escombreras o sitios de disposición final de residuos provenientes de la construcción">
				<input type="hidden" id="tipoTramiteNumOficio" value="28">
				<%@ include
				...
				<form id="formulario" role="form">
					...
					<div class="input-group">
						<span class="input-group-addon">Domicilio para oír y/o
							recibir notificaciones:</span> <select id="idestablecimientomens"
					...
					<h2>Información general del terreno donde se pretende ubicar
						la escombrera:</h2>
					...
					<div class="box-header with-border">
						<h2>Uso del predio al término de vida útil como escombrera:</h2>
					</div>
					<div class="input-group">
						<span class="input-group-addon"></span> <input maxlength="100"
							type="text" id="usprteviutesc" name="usprteviutesc"
							class="form-control">
					</div>
					<br>
					<div class="input-group">
						<span class="input-group-addon">Cantidades de residuos de
							construcción, mantenimiento, demolición y materiales de despalme
							(en metros cúbicos)</span> <input maxlength="100" type="text"
							id="carecomademades" name="carecomademades" class="form-control">
					</div>
					<br>

					<h2>Número de Oficio de MIA:</h2>

					<div class="input-group">
						<span class="input-group-addon"></span> <input maxlength="100"
							type="text" id="nuofmia" name="nuofmia" class="form-control">
					</div>
					<br>
					<div class="input-group">
