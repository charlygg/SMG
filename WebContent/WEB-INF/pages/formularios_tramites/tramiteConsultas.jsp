...
.change(
		function() {
			$
					.getJSON(
							'consultaTramite_consultas?idtramite_consultas='
									+ $(
											"#idtramite_consultas")
											.val(),
							function(beanDatos) {
...
$(document).ready(function() {
		$.validator.setDefaults({submitHandler: function() { transaccionDatosRetornoID( 'registerTramite_consultas', 'formulario', true, "exito()", "fallo()");}});
		$('#formulario').validate();
		comboListaTramite();
		comboEstatus();
	});
...
$('#idtramite_consultas').empty();
$.getJSON('listacomboTramite_consultas?rol_usuario='
		+ $("#rol_usuario").val(), function(beanDatos) {
	console.log(JSON.stringify(beanDatos));
	//datos para el combo
	if ($("#rol_usuario").val() != 'ROLE_RECEP') {
		$("#idtramite_consultas").append(
	...
	$.each(beanDatos, function(i, item) {
		console.log(item.idtramite_consultas);
		var estatus = '';
		switch (item.estatus) {
		case 1:
			estatus = 'Nuevo';
		...
		$("#idtramite_consultas").append(
				"<option value=\""+item.idtramite_consultas+"\">Trámite No "
						+ item.idtramite_consultas
	...
	function exito(){
// 		mensajeriaAlertas('Exito', 'En Alta de Tramite Consultas');
		comboListaTramite();
		$('#idtramite_consultas').eq(1).prop('selected', true);
	...
	function fallo(){
		mensajeriaAlertas('Error', 'En Alta de Tramite Consultas');
		$('#idtramite_consultas').eq(1).prop('selected', true);
		$('#catalogo_perfilempresa_idcatalogo_perfilempresa').eq(0).prop(
				'selected', true);
		$("#catalogo_perfilestablecimiento_idcatalogo_perfilestablecimiento")
	...
	function cargaTablaMovsBitacora(estatus){
		if(estatus == 1){
			...
			$('#divCargaBitacora').load('consultaBitacoraCatReg?idregistro='+$('#idtramite_consultas').val()+'&catalogoID='+$('#tramite_catalogoID').val());
		}
	...
	<div class="col-lg-12">
		<div class="box box-success">
			<div class="box-header with-border">
				<h3 class="box-title">Tramite Consultas.</h3>
			</div>

			<div class="box-body">
				<!-- elemntos que se cambian por tramite -->
				<input type="hidden" id="areaNumOficio" value="IA"> <input
					type="hidden" id="titleNumOficio" value="Tramite Consultas">
				<input type="hidden" id="tipoTramiteNumOficio" value="11">
				<%@ include
						...
						<c:when test="${empty data}">
							<div class="input-group" id="dividtramite">
								<span class="input-group-addon">Trámites en Ejecución</span> <select
									class="form-control" id="idtramite_consultas"
									name="idtramite_consultas"></select>
							</div>
							<br>
						</c:when>
					</c:choose>
