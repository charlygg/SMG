...
$("#idtramite_tratamiento")
.change(
		function() {
			$
					.getJSON(
							'consultaTramite_registro_de_planta_de_tratamiento_de_aguas_residuales?idtramite_tratamiento='
									+ $(
											"#idtramite_tratamiento")
											.val(),
							function(beanDatos) {
								...
								$.getJSON('getArrayEstablecimientosPorEmpresa?idempresa='+$("#idempresa").val(),
												function(beanDatos) {
													...
													$('#idestablecimiento > option[value='+establecimientoASeleccionar+']').attr(
															'selected', 'selected');
												});
...
$(document).ready(function() {
		$.validator.setDefaults({submitHandler: function() { transaccionDatosRetornoID( 'registerTramite_registro_de_planta_de_tratamiento_de_aguas_residuales', 'formulario', true, "exito()", "fallo()");}});
		$('#formulario').validate();
		comboListaTramite();
		comboEstatus();
	});
...
$('#idtramite_tratamiento').empty();
$.getJSON('listacomboTramite_registro_de_planta_de_tratamiento_de_aguas_residuales?rol_usuario='
		+ $("#rol_usuario").val(), function(beanDatos) {
	console.log(JSON.stringify(beanDatos));
	//datos para el combo
	if ($("#rol_usuario").val() != 'ROLE_RECEP') {
		$("#idtramite_tratamiento").append(
	...
	$.each(beanDatos, function(i, item) {
		console.log(item.idtramite_tratamiento);
		var estatus = '';
		switch (item.estatus) {
		case 1:
			estatus = 'Nuevo';
		...
		$("#idtramite_tratamiento").append(
				"<option value=\""+item.idtramite_tratamiento+"\">Trámite No "
						+ item.idtramite_tratamiento
	...
	function exito(){
// 		mensajeriaAlertas('Exito', 'En Alta de Trámite Planta de Tratamento de Agua Residual');
		comboListaTramite();
		$('#idtramite_tratamiento').eq(1).prop('selected', true);
		$('#idempresa').eq(0).prop(
				'selected', true);
		$("#idestablecimiento")
	...
	function fallo(){
		mensajeriaAlertas('Error', 'En Alta de Trámite Planta de Tratamento de Agua Residual');
		$('#idtramite_tratamiento').eq(1).prop('selected', true);
		$('#idempresa').eq(0).prop(
				'selected', true);
		$("#idestablecimiento")
	...
	function cargaTablaMovsBitacora(estatus){
		if(estatus == 1){
			...
			$('#divCargaBitacora').load('consultaBitacoraCatReg?idregistro='+$('#idtramite_tratamiento').val()+'&catalogoID='+$('#tramite_catalogoID').val());
		}
	...
	<div class="col-lg-12">
		<div class="box box-success">
			<div class="box-header with-border">
				<h3 class="box-title">Trámite Planta de Tratamento de Agua
					Residual</h3>
			</div>
			...
			<div class="box-body">
				...
				<input type="hidden" id="areaNumOficio" value="CD"> <input
					...
					value="Trámite Planta de Tratamento de Agua Residual"> <input
					type="hidden" id="tipoTramiteNumOficio" value="17">
				<%@ include
						...
						<c:when test="${empty data}">
							<div class="input-group" id="dividtramite">
								<span class="input-group-addon">Trámites en Ejecución</span> <select
									class="form-control" id="idtramite_tratamiento"
									name="idtramite_tratamiento"></select>
							</div>
							<br>
						</c:when>
					</c:choose>
						...
						<c:otherwise>
							<div class="input-group">
								<span class="input-group-addon">Fecha</span> <input
									maxlength="100" type="text" id="fechaemision"
									name="fechaemision" class="form-control"
									value="${data.fechaemision}" disabled>
							</div>
							<br>
							<div class="input-group">
								<span class="input-group-addon">RFC</span> <input
									maxlength="100" type="text" id="rfc" name="rfc"
									class="form-control" value="${data.rfc}" disabled>
							</div>
							<br>
