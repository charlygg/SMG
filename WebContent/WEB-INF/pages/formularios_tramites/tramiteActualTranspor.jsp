...
$("#catalogo_perfilempresa_idcatalogo_perfilempresa").change(function() {
 ...
 $("#idtramite_actualizacion_tranps").change(function() {
	    $.getJSON('consultaTramite_actualizacion_tranps?idtramite_actualizacion_tranps='+$("#idtramite_actualizacion_tranps").val(),function(data){
	        console.log(JSON.stringify(data));
		...
		function comboListaTramite(){
			$.getJSON('listacomboTramite_actualizacion_tranps?rol_usuario='+$("#rol_usuario").val(),function(data){
		        console.log(JSON.stringify(data));
		        //datos para el combo
		        if($("#rol_usuario").val() != 'ROLE_RECEP'){
		        	$("#idtramite_actualizacion_tranps").append("<option value='0'>Escoja una opción</option>");	
		        }else{
		        	$("#idtramite_actualizacion_tranps").append("<option value='0'>Escoja una opción</option>");
		        }
		        
		        $.each(data, function(i,item){
		        	 console.log(item.idtramite_actualizacion_tranps);
		        	 var estatus = ''; 
		        	switch(item.estatus){
		            ...
		            $("#idtramite_actualizacion_tranps").append("<option value=\""+item.idtramite_actualizacion_tranps+"\">Trámite No "+item.idtramite_actualizacion_tranps+" en Estatus "+estatus+"</option>");
		        });
...
$(document).ready(function() {
		$.validator.setDefaults({submitHandler: function() { transaccionDatosRetornoID( 'registerTramite_actualizacion_tranps', 'formulario', true, "exito()", "fallo()");}});
		$('#formulario').validate();
		comboListaTramite();
		comboEstatus();
});
	...
	function exito(){
// 		mensajeriaAlertas('Exito', 'En Alta de Tramite Actualización de transportistas');
	...
	function fallo(){
		mensajeriaAlertas('Error', 'En Alta de Tramite Actualización de transportistas');
		document.getElementById("catalogo_perfilempresa_idcatalogo_perfilempresa").selectedIndex = 0;
		document.getElementById("catalogo_perfilestablecimiento_idcatalogo_perfilestablecimiento").selectedIndex = -1;
		document.getElementById("estatus").selectedIndex = 0;
		$('#divCargaBitacora').hide();
	...
	if($("#busquedaTramite").val() != 0){
	    ...
	    $.getJSON('consultaTramite_actualizacion_tranps?idtramite_actualizacion_tranps='+$("#busquedaTramite").val(),function(data){
	        console.log(JSON.stringify(data));
	        //cargar datos en la vista:
	        	$('#catalogo_perfilempresa_idcatalogo_perfilempresa > option[value="'+data.catalogo_perfilempresa_idcatalogo_perfilempresa+'"]').attr('selected', 'selected');
	        	///LLenas catalogo de establecimientos
	...
	<div class="col-lg-12">
		<div class="box box-success">
			<div class="box-header with-border">
				<h3 class="box-title">Tramite Actualización de transportistas</h3>
			</div>
			...
			<div class="box-body">
				...
				<input type="hidden" id="areaNumOficio" value="CD"> <input
					...
					value="Tramite Actualización de transportistas"> <input
					type="hidden" id="tipoTramiteNumOficio" value="22">
				<%@ include
				...
				<form id="formulario" role="form">
						...
						<c:when test="${empty data}">
							<div class="input-group" id="dividtramite">
								<span class="input-group-addon">Trámites en Ejecución</span> <select
									class="form-control" id="idtramite_actualizacion_tranps"
									name="idtramite_actualizacion_tranps"></select>
							</div>
						</c:when>
					</c:choose>
					<br>
					...
					<br> <input type="hidden" id="idtramite_actualizacion_tranps"
						name="idtramite_actualizacion_tranps" value="0">
					<!--  -->
					<c:choose>
						<c:when test="${empty data}">
						...
						<c:otherwise>
							...
							<input type="hidden" id="busquedaTramite" name="busquedaTramite"
								value="${data.idtramite_actualizacion_tranps}">
						</c:otherwise>
					</c:choose>
					<!--  -->
					<!-- Termina Agregar -->
