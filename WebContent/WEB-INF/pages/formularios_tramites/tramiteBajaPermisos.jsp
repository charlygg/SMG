...
$("#idtramite_baja_permisos")
.change(
		function() {
			$
					.getJSON(
							'consultaTramite_baja_permisos?idtramite_baja_permisos='
									+ $(
											"#idtramite_baja_permisos")
											.val(),
							function(beanDatos) {
...
$(document).ready(function() {
		$.validator.setDefaults({submitHandler: function() { transaccionDatosRetornoID( 'registerTramite_baja_permisos', 'formulario', true, "exito()", "fallo()");}});
		$('#formulario').validate();
		comboListaTramite();
		comboEstatus();
	});
...
$('#idtramite_baja_permisos').empty();
$.getJSON('listacomboTramite_baja_permisos?rol_usuario='
		+ $("#rol_usuario").val(), function(beanDatos) {
	console.log(JSON.stringify(beanDatos));
	//datos para el combo
	if ($("#rol_usuario").val() != 'ROLE_RECEP') {
		$("#idtramite_baja_permisos").append(
	...
	$.each(beanDatos, function(i, item) {
		console.log(item.idtramite_baja_permisos);
		var estatus = '';
		switch (item.estatus) {
		case 1:
			estatus = 'Nuevo';
		...
		$("#idtramite_baja_permisos").append(
				"<option value=\""+item.idtramite_baja_permisos+"\">Trámite No "
						+ item.idtramite_baja_permisos
	...
	function exito(){
// 		mensajeriaAlertas('Exito', 'En Alta de Tramite Baja de Permisos');
		comboListaTramite();
		$('#idtramite_baja_permisos').eq(1).prop('selected', true);
	...
	function fallo(){
		mensajeriaAlertas('Error', 'En Alta de Tramite Baja de Permisos');
		$('#idtramite_baja_permisos').eq(1).prop('selected', true);
		$('#catalogo_perfilempresa_idcatalogo_perfilempresa').eq(0).prop(
				'selected', true);
		$("#catalogo_perfilestablecimiento_idcatalogo_perfilestablecimiento")
	...
	function cargaTablaMovsBitacora(estatus){
		if(estatus == 1){
			...
			$('#divCargaBitacora').load('consultaBitacoraCatReg?idregistro='+$('#idtramite_baja_permisos').val()+'&catalogoID='+$('#tramite_catalogoID').val());
		}
	...
	<div class="col-lg-12">
		<div class="box box-success">
			<div class="box-header with-border">
				<h3 class="box-title">Tramite Baja de permisos.</h3>
			</div>
						...
						<c:when test="${empty data}">
							<div class="input-group" id="dividtramite">
								<span class="input-group-addon">Trámites en Ejecución</span> <select
									class="form-control" id="idtramite_baja_permisos"
									name="idtramite_baja_permisos"></select>
							</div>
							<br>
