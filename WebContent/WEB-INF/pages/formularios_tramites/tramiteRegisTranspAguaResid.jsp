...
$("#idtramite_registro_trans_aguas_reci").change(
		function() {
			$.getJSON(
				'consultaTramite_registro_de_transportistas_de_aguas_residuales?idtramite_registro_trans_aguas_reci='
				+ $("#idtramite_registro_trans_aguas_reci").val(),
							function(beanDatos) {
								$('#catalogo_perfilempresa_idcatalogo_perfilempresa > option[value="'+ beanDatos.catalogo_perfilempresa_idcatalogo_perfilempresa+ '"]').attr('selected', 'selected');
								...
								$.getJSON('getArrayEstablecimientosPorEmpresa?idempresa='+$("#catalogo_perfilempresa_idcatalogo_perfilempresa").val(),
												function(beanDatos) {
													console.log(JSON.stringify(beanDatos));
													$("#catalogo_perfilestablecimiento_idcatalogo_perfilestablecimiento").empty();
													$.each(beanDatos,function(i,item) {
																		$("#catalogo_perfilestablecimiento_idcatalogo_perfilestablecimiento").append(
...
$(document).ready(function() {
		$.validator.setDefaults({submitHandler: function() { transaccionDatosRetornoID( 'registerTramite_registro_de_transportistas_de_aguas_residuales', 'formulario', true, "exito()", "fallo()");}});
		$('#formulario').validate();
		comboListaTramite();
		comboEstatus();
	});
...
$('#idtramite_registro_trans_aguas_reci').empty();
$.getJSON('listacomboTramite_registro_de_transportistas_de_aguas_residuales?rol_usuario='
		+ $("#rol_usuario").val(), function(beanDatos) {
	console.log(JSON.stringify(beanDatos));
	//datos para el combo
	if ($("#rol_usuario").val() != 'ROLE_RECEP') {
		$("#idtramite_registro_trans_aguas_reci").append(
	...
	$.each(beanDatos, function(i, item) {
		console.log(item.idtramite_registro_trans_aguas_reci);
		var estatus = '';
		switch (item.estatus) {
		case 1:
			estatus = 'Nuevo';
		...
		$("#idtramite_registro_trans_aguas_reci").append(
				"<option value=\""+item.idtramite_registro_trans_aguas_reci+"\">Trámite No "
						+ item.idtramite_registro_trans_aguas_reci
	...
	function exito(){
// 		mensajeriaAlertas('Exito', 'En Alta de Tramite Registro de Transportistas de Aguas Residuales.');
		comboListaTramite();
		$('#idtramite_registro_trans_aguas_reci').eq(1).prop('selected', true);
	...
	function fallo(){
		mensajeriaAlertas('Error', 'En Alta de Tramite Registro de Transportistas de Aguas Residuales.');
		$('#idtramite_registro_trans_aguas_reci').eq(1).prop('selected', true);
		$('#catalogo_perfilempresa_idcatalogo_perfilempresa').eq(0).prop(
				'selected', true);
		$("#catalogo_perfilestablecimiento_idcatalogo_perfilestablecimiento")
	...
	function cargaTablaMovsBitacora(estatus){
		if(estatus == 1){
			...
			$('#divCargaBitacora').load('consultaBitacoraCatReg?idregistro='+$('#idtramite_registro_trans_aguas_reci').val()+'&catalogoID='+$('#tramite_catalogoID').val());
		}
	...
	<div class="col-lg-12">
		<div class="box box-success">
			<div class="box-header with-border">
				<h3 class="box-title">Tramite Registro de Transportistas de
					Aguas Residuales.</h3>
			</div>
			...
			<div class="box-body">
				...
				<input type="hidden" id="areaNumOficio" value="CD"> <input
					...
					value="Tramite Registro de Transportistas de Aguas Residuales">
				<input type="hidden" id="tipoTramiteNumOficio" value="19">
				<%@ include
						...
						<c:when test="${empty data}">
							<div class="input-group" id="dividtramite">
								<span class="input-group-addon">Trámites en Ejecución</span> <select
									class="form-control" id="idtramite_registro_trans_aguas_reci"
									name="idtramite_registro_trans_aguas_reci"></select>
							</div>
						</c:when>
					</c:choose>
					<br>
