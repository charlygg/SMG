...
$("#idtramite_modif_licencia")
.change(
		function() {
			$
					.getJSON(
							'consultaTramite_modif_licencia?idtramite_modif_licencia='
									+ $(
											"#idtramite_modif_licencia")
											.val(),
							function(beanDatos) {
...
$(document).ready(function() {
		$.validator.setDefaults({submitHandler: function() { transaccionDatosRetornoID( 'registerTramite_modif_licencia', 'formulario', true, "exito()", "fallo()");}});
		$('#formulario').validate();
		comboListaTramite();
		comboEstatus();
	});
...
$('#idtramite_modif_licencia').empty();
$.getJSON('listacomboTramite_modif_licencia?rol_usuario='
		+ $("#rol_usuario").val(), function(beanDatos) {
	console.log(JSON.stringify(beanDatos));
	//datos para el combo
	if ($("#rol_usuario").val() != 'ROLE_RECEP') {
		$("#idtramite_modif_licencia").append(
	...
	$.each(beanDatos, function(i, item) {
		console.log(item.idtramite_modif_licencia);
		var estatus = '';
		switch (item.estatus) {
		case 1:
			estatus = 'Nuevo';
		...
		$("#idtramite_modif_licencia").append(
				"<option value=\""+item.idtramite_modif_licencia+"\">Trámite No "
						+ item.idtramite_modif_licencia
	...
	function exito(){
// 		mensajeriaAlertas('Exito', 'En Alta de Tramite Modificación de Licencia de Funcionamiento');
		comboListaTramite();
		$('#idtramite_modif_licencia').eq(1).prop('selected', true);
	...
	function fallo(){
		mensajeriaAlertas('Error', 'En Alta de Tramite Modificación de Licencia de Funcionamiento');
		$('#idtramite_modif_licencia').eq(1).prop('selected', true);
		$('#catalogo_perfilempresa_idcatalogo_perfilempresa').eq(0).prop(
				'selected', true);
		$("#catalogo_perfilestablecimiento_idcatalogo_perfilestablecimiento")
	...
	function cargaTablaMovsBitacora(estatus){
		if(estatus == 1){
			...
			$('#divCargaBitacora').load('consultaBitacoraCatReg?idregistro='+$('#idtramite_modif_licencia').val()+'&catalogoID='+$('#tramite_catalogoID').val());
		}
	...
	<div class="col-lg-12">
		<div class="box box-success">
			<div class="box-header with-border">
				<h3 class="box-title">Tramite Modificación de licencia.</h3>
			</div>
						...
						<c:when test="${empty data}">
							<div class="input-group" id="dividtramite">
								<span class="input-group-addon">Trámites en Ejecución</span> <select
									class="form-control" id="idtramite_modif_licencia"
									name="idtramite_modif_licencia"></select>
							</div>
							<br>
