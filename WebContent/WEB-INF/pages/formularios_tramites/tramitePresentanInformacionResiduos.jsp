	...
	<div class="col-lg-12">
		<div class="box box-success">
			<div class="box-header with-border">
				<h3 class="box-title">Trámite Presentan Información Residuos</h3>
			</div>
				...
				<c:choose>
					<c:when test="${empty data }">
						<div class="input-group" id="dividtramite">
							<span class="input-group-addon">Trámites en Ejerución</span>
							<select class="form-control" id="idtramite_residuos"
								name="idtramite_residuos"></select>
						</div>
						<br>
						<div class="input-group">
							...
							<c:choose>
								<c:when test="${!empty data }">
									...
									class="form-control" required>${data.asunto}</textarea>
								</c:when>
								<c:otherwise>
									<textarea id="asunto" name="asunto"
						...
						<br>
						<div class="input-group">
							<span class="input-group-addon">Fecha de recepción de documentación de Trámite</span>
							<c:choose>
								<c:when test="${!empty data }">
				...
				<div id="DatosEstablecimiento" class="container" hidden>
					...
					<c:choose>
						...
						<div class="row">
								<div class="col-lg-6">
									<div class="input-group">
										<span class="input-group-addon">Notificado</span>
...
$(document).ready(function(){
	...
	$("#idtramite_residuos").change(function() {
		$.getJSON(
			'consultaTramite_presentan_info_residuos?idtramite_residuos='
			+ $("#idtramite_residuos").val(),
						function(beanDatos) {
	...
	$.validator.setDefaults({submitHandler: function() { transaccionDatosRetornoID( 'registerTramite_presentan_info_residuos', 'formulario', true, "exito()", "fallo()");}});
	$('#formulario').validate();
	comboListaTramite();
	comboEstatus();
...
function comboListaTramite(){
	$("#idtramite_residuos").empty();
	
	$.getJSON('listacomboTramite_presentan_info_residuos?rol_usuario='
			+ $("#rol_usuario").val(), function(beanDatos) {
		console.log(JSON.stringify(beanDatos));
		//datos para el combo
			if ($("#rol_usuario").val() != 'ROLE_RECEP') {
		$("#idtramite_residuos").append(
				"<option value=''>Escoja una opción</option>");
	} else {
		$("#idtramite_residuos").append(
			...
			$.each(beanDatos, function(i, item) {
				console.log(item.idtramite_residuos);
				var estatus = '';
				switch (item.estatus) {
				case 1:
					estatus = 'Nuevo';
				...
				$("#idtramite_residuos").append(
						"<option value=\""+item.idtramite_residuos+"\">Trámite No "
								+ item.idtramite_residuos
...
function exito(){
// 	mensajeriaAlertas('Exito', 'En Alta de Tramite Presentan Información Residuos de Manejo Especial');
	comboListaTramite();
	$('#idtramite_residuos').eq(1).prop('selected', true);
	$('#idempresa').eq(0).prop(
			'selected', true);
	$("#idestablecimiento")
...
function fallo(){
	mensajeriaAlertas('Error', 'En Alta de Tramite Presentan Información Residuos de Manejo Especial');
	$('#idtramite_residuos').eq(1).prop('selected', true);
	$('#idempresa').eq(0).prop(
			'selected', true);
	$("#idestablecimiento")
...
function cargaTablaMovsBitacora(estatus){
	if(estatus == 1){
		...
		$('#divCargaBitacora').load('consultaBitacoraCatReg?idregistro='+$('#idtramite_residuos').val()+'&catalogoID='+$('#tramite_catalogoID').val());
	}
}
