...
$("#idtramite_solicitud_copias")
.change(
		function() {
			$
					.getJSON(
							'consultaTramite_solicitud_copias?idtramite_solicitud_copias='
									+ $(
											"#idtramite_solicitud_copias")
											.val(),
							function(beanDatos) {
...
$(document).ready(function() {
		$.validator.setDefaults({submitHandler: function() { transaccionDatosRetornoID( 'registerTramite_solicitud_copias', 'formulario', true, "exito()", "fallo()");}});
		$('#formulario').validate();
		comboListaTramite();
		comboEstatus();
	});
...
$('#idtramite_solicitud_copias').empty();
$.getJSON('listacomboTramite_solicitud_copias?rol_usuario='
		+ $("#rol_usuario").val(), function(beanDatos) {
	console.log(JSON.stringify(beanDatos));
	//datos para el combo
	if ($("#rol_usuario").val() != 'ROLE_RECEP') {
		$("#idtramite_solicitud_copias").append(
	...
	$.each(beanDatos, function(i, item) {
		console.log(item.idtramite_solicitud_copias);
		var estatus = '';
		switch (item.estatus) {
		case 1:
			estatus = 'Nuevo';
		...
		$("#idtramite_solicitud_copias").append(
				"<option value=\""+item.idtramite_solicitud_copias+"\">Trámite No "
						+ item.idtramite_solicitud_copias
	...
	function exito(){
// 		mensajeriaAlertas('Exito', 'En Alta de Tramite Solicitud de Copias');
		comboListaTramite();
		$('#idtramite_solicitud_copias').eq(1).prop('selected', true);
	...
	function fallo(){
		mensajeriaAlertas('Error', 'En Alta de Tramite Solicitud de Copias');
		$('#idtramite_solicitud_copias').eq(1).prop('selected', true);
		$('#catalogo_perfilempresa_idcatalogo_perfilempresa').eq(0).prop(
				'selected', true);
		$("#catalogo_perfilestablecimiento_idcatalogo_perfilestablecimiento")
	...
	function cargaTablaMovsBitacora(estatus){
		if(estatus == 1){
			...
			$('#divCargaBitacora').load('consultaBitacoraCatReg?idregistro='+$('#idtramite_solicitud_copias').val()+'&catalogoID='+$('#tramite_catalogoID').val());
		}
	...
	<div class="col-lg-12">
		<div class="box box-success">
			<div class="box-header with-border">
				<h3 class="box-title">Tramite Solicitud de Copias.</h3>
			</div>
			...
			<div class="box-body">
				...
				<input type="hidden" id="areaNumOficio" value="IA"> <input
					...
					value="Tramite Solicitud de Copias"> <input type="hidden"
					id="tipoTramiteNumOficio" value="10">
				<%@ include
						...
						<c:when test="${empty data}">
							<div class="input-group" id="dividtramite">
								<span class="input-group-addon">Trámites en Ejecución</span> <select
									class="form-control" id="idtramite_solicitud_copias"
									name="idtramite_solicitud_copias"></select>
							</div>
							<br>
						</c:when>
					</c:choose>
